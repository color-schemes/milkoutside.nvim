...
local Groups = require("milkoutside.groups")
...
function M.extras()
  local Extra = require("milkoutside.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/milkoutside/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/milkoutside/config.lua", "\n(--@class milkoutside%.Config.-\n})")
