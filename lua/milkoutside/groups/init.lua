...
function M.get_group(name)
  ---@type {get: milkoutside.HighlightsFn, url: string}
  return Util.mod("milkoutside.groups." .. name)
end
...
function M.setup(colors, opts)
  ...
  if not ret then
    ...
    Util.resolve(ret)
    if opts.cache then
      local cache_entry = { groups = ret, inputs = inputs }
      local ok, err = pcall(Util.cache.write, cache_entry)
      if not ok then
